...
export const FoodPopularBiryani: React.FC<FoodPopularBiryaniProps> = ({
  onAddToCart,
  onViewAllPress,
}) => {
  ...
  const filteredProducts =
    activeFilter === 'All Dishes'
      ? BIRYANI_PRODUCTS
      : BIRYANI_PRODUCTS.filter((p) => p.categoryTag === activeFilter);

  return (
    <View className="my-6 w-full relative">
      {/* 1. Header Row - 1 Line Layout */}
      <View className="flex-row items-center justify-between px-4 mb-2 gap-1.5 w-full">
        <Text
          numberOfLines={1}
      ...
      <ScrollView
        ...
        className="mb-3"
      >
        {['All Dishes', 'Kacchi', 'Tehari', 'Polao & Chicken'].map((cat) => {
          ...
          return (
            <TouchableOpacity
              ...
              className={`px-3.5 py-1.5 rounded-full border shadow-xs ${
                isActive
