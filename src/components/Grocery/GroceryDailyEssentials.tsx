...
export interface DailyEssentialsProduct {
  id: string;
  brand: string;
  title: string;
  imageUrl: string;
...
export const GroceryDailyEssentials: React.FC<GroceryDailyEssentialsProps> = ({
  onAddToCart,
  onViewAllPress,
}) => {
  ...
  return (
    <View className="my-6 w-full relative">
      ...
      <View className="flex-row items-start justify-between px-4 mb-2 gap-1.5 w-full">
        {/* Left Side: Bag Icon + Title + Count Badge */}
        <View className="flex-1 min-w-0 shrink">
          <View className="flex-row items-center gap-2 min-w-0 shrink">
            <View className="w-8 h-8 rounded-xl bg-amber-100 dark:bg-amber-950 items-center justify-center shrink-0">
            ...
            >
              {t('dailyEssentialsTitle')}
            </Text>
            <View className="bg-amber-100 dark:bg-amber-950 px-2.5 py-0.5 rounded-full shrink-0">
              <Text className="text-amber-700 dark:text-amber-400 text-[11px] font-extrabold">
                {t('dailyEssentialsCount')}
              </Text>
            </View>
          </View>
          <Text
      ...
      >
        {DAILY_ESSENTIALS_PRODUCTS.map((item) => {
