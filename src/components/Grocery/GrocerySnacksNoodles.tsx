...
export const GrocerySnacksNoodles: React.FC<GrocerySnacksNoodlesProps> = ({
  onAddToCart,
  onViewAllPress,
}) => {
  ...
  const cardStep = width >= 640 ? 246 : 168; // Card width + gap
  ...
  const handleScrollNext = () => {
    const maxScroll = (SNACKS_PRODUCTS.length - 1) * cardStep;
    const targetX = Math.min(maxScroll, scrollX.current + cardStep);
    scrollX.current = targetX;
    scrollRef.current?.scrollTo({ x: targetX, animated: true });
  };
  ...
  return (
    <View className="my-6 w-full relative">
      ...
      {/* 1. Header Row (Strictly 1 single line header row; title text breaks into 2 lines if needed on mobile) */}
      <View className="flex-row items-center justify-between px-4 mb-4 gap-2">
        {/* Left Side: Title (max 2 lines) + Count Badge */}
        ...
        {/* Right Side: Prev / Next 1-Card Step Arrows & View All Button */}
      ...
      {/* 2. Product Cards Carousel - E-Commerce Style Card Layout */}
      <ScrollView
        ref={scrollRef}
        horizontal
      ...
      >
        {SNACKS_PRODUCTS.map((item) => {
          ...
          return (
            ...
            >
              {/* Product Image Box - Full Width to Top, Left, Right Edge with Top Rounded Radius */}
              <View className="w-full h-[165px] sm:h-[185px] bg-slate-50 dark:bg-slate-900 relative items-center justify-center overflow-hidden">
                {/* Optional Discount Ribbon Badge (Top Left Edge) */}
                {activeDiscountText ? (
