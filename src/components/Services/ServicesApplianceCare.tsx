...
export interface ApplianceServiceItem {
  id: string;
  categoryTag: string;
  titleKey: string;
  defaultTitle: string;
...
export const ServicesApplianceCare: React.FC<ServicesApplianceCareProps> = ({
  onServicePress,
}) => {
  ...
  const handleScrollNext = () => {
    const maxScroll = (APPLIANCE_SERVICES.length - 1) * cardWidth;
  ...
  return (
    <View className="my-8 w-full max-w-[1100px] self-center">
      ...
      <View className="flex-row items-center justify-between px-4 mb-5">
        ...
        >
          {t('acApplianceCareTitle') !== 'acApplianceCareTitle'
            ? t('acApplianceCareTitle')
            : 'AC Repair & Home Appliance Care'}
        </Text>
