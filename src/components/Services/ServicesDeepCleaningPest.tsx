...
export interface CleaningPestServiceItem {
  id: string;
  categoryTag: string;
  titleKey: string;
  defaultTitle: string;
...
export const ServicesDeepCleaningPest: React.FC<ServicesDeepCleaningPestProps> = ({
  onServicePress,
}) => {
  ...
  const handleScrollNext = () => {
    const maxScroll = (CLEANING_PEST_SERVICES.length - 1) * cardWidth;
  ...
  return (
    <View className="my-8 w-full max-w-[1100px] self-center">
      ...
      <View className="flex-row items-center justify-between px-4 mb-5">
        ...
        >
          {t('cleaningPestTitle') !== 'cleaningPestTitle'
            ? t('cleaningPestTitle')
            : 'Home Deep Cleaning & Pest Eradication'}
        </Text>
      ...
      >
        {CLEANING_PEST_SERVICES.map((item) => (
          <TouchableOpacity
            key={item.id}
            activeOpacity={0.92}
