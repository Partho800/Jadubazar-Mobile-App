...
export interface PersonalCareServiceItem {
  id: string;
  categoryTag: string;
  titleKey: string;
  defaultTitle: string;
...
export const ServicesPersonalCareSalon: React.FC<ServicesPersonalCareSalonProps> = ({
  onServicePress,
}) => {
  ...
  const handleScrollNext = () => {
    const maxScroll = (PERSONAL_CARE_SERVICES.length - 1) * cardWidth;
  ...
  return (
    <View className="my-8 w-full max-w-[1100px] self-center">
      ...
      <View className="flex-row items-center justify-between px-4 mb-5">
        ...
        >
          {t('personalSalonTitle') !== 'personalSalonTitle'
            ? t('personalSalonTitle')
            : 'Personal Care & Home Salon'}
        </Text>
      ...
      >
        {PERSONAL_CARE_SERVICES.map((item) => (
          <TouchableOpacity
            key={item.id}
            activeOpacity={0.92}
